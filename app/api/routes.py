...
def get_list(list_id):
    ...
    return jsonify([list_.to_dict(args['offset'], args['limit'], args['start_today'])]), 200
...
def delete_share(list_id, share_id):
    ...
    share = ListPermission.query.filter_by(
        id=share_id, list_id=list_id).first()
    if not share:
...
def put_meals(list_id):
    ...
    for index, req_meal in enumerate(req):
        if 'id' in req_meal and req_meal['id'] in [i.id for i in meals]:
            ...
            new_meal = Meal(list_id=list_.id,
                            name=req_meal['name'], order=index)
